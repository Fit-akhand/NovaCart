...
const ADMIN_PREFIXES = ['/create', '/users', '/edit_user', '/categories']
  ...
  if (isAdmin && isAdminRoute) {
    return (
      <div className="flex min-h-screen min-w-0 w-full flex-col bg-[var(--nova-bg)] text-[var(--nova-text)]">
        <AdminHeader onMenuClick={() => setAdminMenuOpen(true)} />
        <Notify />
        <Modal />
        <div className="flex min-h-0 flex-1">
          <AdminSidebar open={adminMenuOpen} onClose={() => setAdminMenuOpen(false)} />
